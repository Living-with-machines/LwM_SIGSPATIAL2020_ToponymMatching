...
* `processing/toponym_matching_datasets/wikigaz/generate_wikigaz_comb.py`: This script assumes the user has already generated the WikiGazetteers in steps [`generate_wikigazetteers.ipynb`](https://github.com/Living-with-machines/LwM_SIGSPATIAL2020_ToponymMatching/blob/tm_experiments/processing/gazetteers/generate_wikigazetteers.ipynb) and [`prepare_gazetteers.ipynb`](https://github.com/Living-with-machines/LwM_SIGSPATIAL2020_ToponymMatching/blob/tm_experiments/processing/gazetteers/prepare_gazetteers.ipynb). This results in three dataframes (`wikiGaz_en_filtered.pkl`, `wikiGaz_es_filtered.pkl`, and `wikiGaz_el_filtered.pkl`, stored in `processing/resources/`) with the following columns: `index`, `name`, `wikititle`, `latitude`, `longitude`, `source`.
