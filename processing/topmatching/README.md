...
* `wikigaz/generate_wikigaz_dataset.ipynb` (python script: `wikigaz/generate_dataset.py`): This notebook generates an equal number of positive and negative pairs from a list of toponyms. Positive candidates are derived from WikiGazetteer alternative names. To generate the negative candidates given a place name, we first divide it in ngrams, then retrieve all altnames that contain one of the ngrams and finally rank them using Levensthein distance. We filter out altnames that can correspond to locations within 50 km distance from the source place name.